...
public class RedisService<T>   {
  ...
  public String set(String key, T value, String nxxx, String expx, Integer seconds) {
    if(null != seconds && 0 < seconds &&
      ...
      return jedisCluster.set(key, JSONUtils.toJsonString(value, Object.class), nxxx, expx, seconds);
    } else {
      // 说明不需要设置失效时间
      // 但是仍然需要进一步去判断用户所传递到底nx还是xx
      if(NX.equals(nxxx)) {
        return String.valueOf(jedisCluster.setnx(key, JSONUtils.toJsonString(value, Object.class)));
      } else if(XX.equals(nxxx)) {
        return jedisCluster.set(key, JSONUtils.toJsonString(value, Object.class));
      }
    }
    return NO;
  }
  ...
  public T getOne(String Key){
    return (T) JSONUtils.toJsonString(jedisCluster.get(Key), Object.class);
  }
